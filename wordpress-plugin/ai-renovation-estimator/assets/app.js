(function () {
  ...
  function generateResults() {
    ...
    // Submit lead to WordPress via AJAX (only when running inside the plugin)
    if (typeof window.aireConfig !== 'undefined' && !state._submitted) {
      state._submitted = true;
      var formData = new FormData();
      formData.append('action', 'aire_submit_lead');
      formData.append('nonce', window.aireConfig.nonce);
      formData.append('name', state.name);
      formData.append('email', state.email);
      formData.append('whatsapp', state.whatsapp);
      formData.append('property', state.propertyType || '');
      formData.append('total', total);
      state.scopeOfWorks.forEach(function (s) { formData.append('scopes[]', s.value); });
      state.designStyles.forEach(function (s) { formData.append('styles[]', s); });

      fetch(window.aireConfig.ajaxUrl, {
        method: 'POST',
        body: formData,
